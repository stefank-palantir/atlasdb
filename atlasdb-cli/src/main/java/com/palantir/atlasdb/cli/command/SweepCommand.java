...
public class SweepCommand extends SingleBackendCommand {
    ...
    @Option(name = {"--column-batch--size"},
            description = "Sweeper column batch size (default : no limit), used only for Cassandra KVS")
    Integer columnBatchSize;
	...
	public int execute(final AtlasDbServices services) {
        ...
        for (Map.Entry<TableReference, Optional<byte[]>> entry : tableToStartRow.entrySet()) {
            ...
            while (startRow.isPresent()) {
                ...
                SweepResults results = sweepRunner.run(table, batchSize, columnBatchSize, startRow.get());
