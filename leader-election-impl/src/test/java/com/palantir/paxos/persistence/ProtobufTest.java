...
public class ProtobufTest {
    ...
    public void testPaxosValuePersistence() throws Exception {
        ...
        expected = new PaxosValue(PaxosInstanceId.fromSeq(2), new byte[]{8, 0, 1, 2, 5});
        persisted = expected.persistToBytes();
        actual = PaxosValue.hydrateFromProto(PaxosPersistence.PaxosValue.parseFrom(persisted));
        assertEquals(expected, actual);

        expected = new PaxosValue(PaxosInstanceId.fromSeq(2), null);
        persisted = expected.persistToBytes();
        actual = PaxosValue.hydrateFromProto(PaxosPersistence.PaxosValue.parseFrom(persisted));
        assertEquals(expected, actual);
    }
    ...
    public void testPaxosProposalPersistence() throws Exception {
        ...
        expected = new PaxosProposal(new PaxosProposalId(55, "nonce"), new PaxosValue(PaxosInstanceId.fromSeq(93), null));
        persisted = expected.persistToProto();
        actual = PaxosProposal.hydrateFromProto(persisted);
        assertEquals(expected, actual);

        expected = new PaxosProposal(new PaxosProposalId(0, "noice"), new PaxosValue(PaxosInstanceId.fromSeq(93), new byte[]{}));
        persisted = expected.persistToProto();
        actual = PaxosProposal.hydrateFromProto(persisted);
        assertEquals(expected, actual);
    }
    ...
    public void testPaxosPromisePersistence() throws Exception {
        ...
        expected = PaxosPromise.accept(new PaxosProposalId(20, "id"), new PaxosProposalId(6, "fire"), new PaxosValue(PaxosInstanceId.fromSeq(5), new byte[]{8, 8, 100}));
        checkPersistence(expected);

        expected = PaxosPromise.accept(new PaxosProposalId(20, "id"), null, new PaxosValue(PaxosInstanceId.fromSeq(5), new byte[]{8, 8, 100}));
        checkPersistence(expected);
    ...
    private void checkPersistence(PaxosPromise expected) {
        PaxosPromiseProto persisted = PaxosPromises.toProto(expected);
        PaxosPromise actual = PaxosPromises.fromProto(persisted);
        assertEquals(expected, actual);
    }
